...
消息进入
  ├─ @-mention
  │    ├─ 检查沉默期@充能（上限2，每15分钟恢复1）
  │    ├─ 检查活跃期@疲劳（连续@越多忽略概率越高）
...
LLM 决策 → 过滤（禁词+风控）→ 去名字前缀 → 打字延迟 → 分条发送
```
...
- 性格：二次元、软糯、社恐
- 风格：口语化、不语c、不在括号加旁白
- 情绪：每次随机心情（happy/sleepy/shy/emo/angry）
- 好奇心：会追问、会主动分享
- 被质疑是 AI：不死不承认
...
| `say [群号] <消息>` | 以 bot 身份发消息 |
| `engage [群号] [0-100]` | 查看/设置参与度 |
| `session start <群号>` | 手动开启活跃期 |
