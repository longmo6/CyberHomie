...
CyberHomie 拥有分层记忆架构，所有记忆均为结构化格式（`[重要度] 内容`），按重要度分级（0.1-1.0），注入 LLM 时按分数排序截断（800 字），超 1500 字自动压缩。

**个体记忆**（`data/memory/<qq_id>.md`）
为每个互动过的人维护档案，包含个人信息和**与 bot 的互动方式**：

```
[0.9] 性格：话多，喜欢接梗，二次元浓度高
[0.8] 兴趣：动漫、游戏、深夜闲聊
[0.8] 和我互动模式：经常互怼，开玩笑为主，我被怼了会委屈
[0.7] ta 喜欢逗我，我有时候会害羞
[0.5] 最近在忙毕设
```

**群体记忆**（`data/group_memory/<group_id>.md`）
每个群的集体认知，包含群信息和 **bot 在该群的行为习惯**：

```
[0.9] 群氛围：二次元为主，经常深夜聊天
[0.9] 我在这个群：话多，爱接梗，经常和泷墨互怼
[0.8] 核心人物：泷墨（话多）、xxx（潜水）
[0.8] 我的习惯：这个群比其他群更活跃，会主动开话题
[0.7] 群梗：xxx
```

**动态行为**
不同群有不同的行为模式，不同人有不同的互动方式。这些不是手动配置的，而是 LLM 从聊天记录中自动学习的：

- 在 A 群话多爱开玩笑，在 B 群沉默认真
- 和用户 A 互怼，和用户 B 认真，和用户 C 撒娇
- 每个群有独立的说话风格和习惯
